...
exports.modificarContratacion = async function (contrato) { 
    ...
    try {
        ...
        throw Error("Error occurred while Finding the Contrato");
    }
    ...
    // 🔍 Verificar qué valor llega en fecha3
    let fecha3 = contrato.fecha3;
    console.log("📌 Fecha3 recibida en modificarContratacion:", fecha3);

    // ✅ Manejo seguro de fecha3
    if (!fecha3 || fecha3 === "" || fecha3 === "null" || fecha3 === "undefined") {
        fecha3 = null;
    } else {
        try {
    ...
    oldContrato.fecha3 = fecha3 || oldContrato.fecha3;
    ...
    // ✅ Evitar notificaciones si fecha3 es nula o demasiado cercana a "ahora"
    const ahora = new Date();
    let notificacion24h = null;
    let notificacion1h = null;
    ...
    if (fecha3 && fecha3 > ahora) {  
        ...
        console.log(`🕒 Horas restantes para fecha3: ${horasRestantes}`);
        console.log(`📅 Nueva notificación 24h: ${notificacion24h}`);
        console.log(`📅 Nueva notificación 1h: ${notificacion1h}`);

        oldContrato.enviada24h = false;
        oldContrato.enviada1h = false;
    } else {
        console.warn("⚠️ No se programarán notificaciones porque fecha3 es nula o está demasiado cerca.");
    }

    oldContrato.notificaciones.notificacion24h = notificacion24h;
    oldContrato.notificaciones.notificacion1h = notificacion1h;

    try {
        var savedContrato = await oldContrato.save();
        console.log("✅ Contrato modificado correctamente:", savedContrato);
        return savedContrato;
    } catch (e) {
        console.error("🚨 Error al actualizar contrato:", e);
        throw Error("An error occurred while updating the Contrato");
    }
};
